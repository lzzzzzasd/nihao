...
public:
    ...
    void ShowMenu(CPlayer& player, Storage& inventory) {
        ...
        while (inStore) {
            ...
            if (command == "药水" || command == "yaoshui") {
                BuyPotion(player, inventory);
            }
            else if (command == "枪" || command == "qiang") {
                BuyGun(player, inventory);
            }
            else if (command == "盔甲" || command == "kuijia") {
                BuyArmor(player, inventory);
            }
            else if (command == "刀" || command == "dao") {
                BuyKnife(player, inventory);
            }
            else if (command == "离开" || command == "likai") {
                inStore = false;
                std::cout << "感谢光临！" << std::endl;
            }
            else {
