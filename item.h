...
public:
    ...
    // 获取所有属性
    const std::map<AttributeType, int>& getAllAttributes() const {
        return m_attributes;
    }
    ...
    void displayInfo() const {
        ...
        std::cout << "类型: " << static_cast<int>(m_type) << std::endl;
        std::cout << "描述: " << m_description << std::endl;
        for (const auto& attr : m_attributes) {
            std::cout << "属性(" << static_cast<int>(attr.first) << "): " << attr.second << std::endl;
        }
    }
};
