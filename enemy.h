...
public:
    ...
    // BOSS构造：直接指定生命值
    CEnemy(const std::string& name, float hp, float attack, float defense, float agility,
        const std::string& skillName, float skillMultiplier, const std::string& skillDesc, float defenseReduction = 0.0f)
        : CRole(name, hp),
        m_type(EnemyType::BOSS),
        m_attack(attack), m_defense(defense), m_agility(agility),
        m_expReward(4), m_goldReward(10), m_bossSkillUsed(false), m_defenseReduction(defenseReduction) {
    ...
    float GetDefenseReduction() const { return m_defenseReduction; }
    void SetDefenseReduction(float reduction) { m_defenseReduction = reduction; }

    // 获取BOSS掉落物品名称
    std::string GetBossDropItem() const {
        if (m_name == "洞穴魔王") {
            return "魔王战斧";
        }
        return "";
